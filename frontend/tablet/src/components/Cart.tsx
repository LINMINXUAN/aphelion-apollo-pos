...
export default function Cart() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // For now, we just rely on the UI resetting
      clearCart();
    } catch (error) {
      alert('訂單送出失敗，請稍後再試');
    }
    ...
    <aside className="w-96 bg-sand-50 border-l border-sand-200 flex flex-col h-full shadow-soft z-20">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-sand-50">
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <div
              key={item.productId}
              className="pos-card p-4 flex flex-col gap-2 animate-flash"
            >
      ...
      <div className="p-4 bg-white/80 border-t border-sand-200 shadow-soft">
        <div className="flex justify-between items-end mb-4">
          ...
          <span className="text-4xl font-semibold text-brand-600">NT$ {totalAmount()}</span>
        </div>
